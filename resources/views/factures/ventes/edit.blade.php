...
<div class="max-w-7xl mx-auto">
    ...
    <form action="{{ route('factures.ventes.update', $facture->id) }}" method="POST" id="formFacture" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-gray-200/60 shadow-[0_4px_25px_rgba(0,0,0,0.04)] p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="w-full border border-gray-200 bg-gray-50 rounded-lg px-3 py-2.5 text-sm text-gray-700 flex items-center gap-2">
                        <i class="fa-solid fa-lock text-gray-400 text-xs"></i> {{ $facture->nom_client }}
                    </div>
                </div>

                <!-- Date d'émission : non modifiable -->
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Date d'émission</label>
                    <div class="w-full border border-gray-200 bg-gray-50 rounded-lg px-3 py-2.5 text-sm text-gray-700 flex items-center gap-2">
                        <i class="fa-solid fa-lock text-gray-400 text-xs"></i> {{ $facture->dateEmissionFacture->format('d/m/Y') }}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="date_echeance" required
                           min="{{ $facture->dateEmissionFacture->format('Y-m-d') }}"
        ...
        <div class="bg-white rounded-2xl border border-gray-200/60 shadow-[0_4px_25px_rgba(0,0,0,0.04)] overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-gray-100">
                <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">Prestations / Services facturés</h2>
            </div>
            ...
            @php
                ...
                if (empty($anciennesLignes)) {
                    $anciennesLignes = [['reference' => '', 'designation' => '', 'description' => '', 'quantite' => 1, 'unite' => '', 'prix_unitaire' => '', 'taux_tva' => 19]];
                }
            @endphp

            <div class="overflow-x-auto">
                <table class="w-full min-w-[1100px]" id="tableLignes">
                    ...
                    <tbody id="corpsLignes" class="divide-y divide-gray-100">
                        @foreach($anciennesLignes as $i => $ligne)
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <input type="text" name="lignes[{{ $i }}][designation]" required
                                ...
                                <td class="px-4 py-3">
                                    <input type="text" name="lignes[{{ $i }}][description]"
                                ...
                                <td class="px-4 py-3">
                                    <input type="text" name="lignes[{{ $i }}][unite]"
                                           value="{{ $ligne['unite'] ?? '' }}"
                                           class="w-20 border border-gray-300 rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500">
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <input type="number" step="0.01" name="lignes[{{ $i }}][taux_tva]"
                                           value="{{ $ligne['taux_tva'] ?? 19 }}" required
                                           class="ligne-tva w-20 border border-gray-300 rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500">
                                </td>
                                <td class="px-4 py-3 ligne-montant text-sm font-medium text-gray-700 whitespace-nowrap">0.00 DT</td>
                                <td class="px-4 py-3">
...
<script>
    ...
    function recalculerTotaux() {
        ...
        document.querySelectorAll('#corpsLignes tr').forEach(function (ligne) {
            ...
            const tva = parseFloat(ligne.querySelector('.ligne-tva')?.value) || 0;
            const montant = qte * prix;
            const montantTvaLigne = montant * (tva / 100);
    ...
    document.getElementById('btnAjouterLigne').addEventListener('click', function () {
        ...
        ligne.querySelectorAll('input').forEach(function (champ) {
            ...
            if (champ.name.includes('quantite')) champ.value = '1';
            else if (champ.name.includes('taux_tva')) champ.value = '19';
            else champ.value = '';
        });
    ...
    document.getElementById('formFacture').addEventListener('input', function (e) {
        if (e.target.matches('.ligne-qte, .ligne-prix, .ligne-tva')) recalculerTotaux();
    });
