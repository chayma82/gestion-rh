...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div>
            ...
            <p class="mt-1 text-gray-500 text-sm">
                Suivre les primes accordées aux employés.
            </p>
        </div>
        <div class="flex gap-3">
            <x-annuler />
        <a href="{{ route('employes.primes.create') }}"
    ...
    <!-- Recherche & filtres -->
    <form action="{{ route('employes.primes.index') }}" method="GET" class="flex flex-col md:flex-row gap-3 mb-6">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @forelse($primes as $prime)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <span class="font-semibold text-gray-900 text-sm">{{ $prime->employe->nom_complet ?? '_' }}</span>
                            </td>

                            <td class="px-6 py-4 text-sm text-gray-600">
                                {{ $prime->date_prime->format('d/m/Y') }}
                            </td>
    ...
    <!-- Pagination -->
    @if(isset($primes) && $primes instanceof \Illuminate\Pagination\LengthAwarePaginator)
        <div class="flex justify-between items-center mt-5">
            <span class="text-sm text-gray-500">
                Affichage {{ $primes->firstItem() ?? 0 }} - {{ $primes->lastItem() ?? 0 }} sur {{ $primes->total() }} primes
            </span>
            <div class="flex items-center gap-2">
                {{ $primes->onEachSide(1)->links() }}
            </div>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mt-8">
        <div class="bg-white rounded-2xl border border-gray-200/60 shadow-[0_4px_25px_rgba(0,0,0,0.04)] p-5 flex items-center gap-4">
            ...
            <div>
                <h4 class="text-xs text-gray-500 uppercase tracking-wide">Total Primes</h4>
                <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ number_format($totalPrimes ?? 0, 2) }} DT</p>
            </div>
        </div>
    </div>
