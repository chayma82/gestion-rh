...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Fournisseurs archivés</h1>
            <p class="mt-1 text-gray-500 text-sm">Consulter et restaurer les fournisseurs archivés.</p>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($fournisseurs as $fournisseur)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <form action="{{ route('fournisseurs.desarchiver', $fournisseur->id) }}" method="POST"
                                    onsubmit="return confirm('Restaurer ce fournisseur ?');">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3 text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">Aucun fournisseur archivé</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div class="bg-white rounded-2xl border border-gray-200/60 shadow-[0_4px_25px_rgba(0,0,0,0.04)] p-5 flex items-center gap-4 mt-8 w-fit">
        ...
        <div>
            <h4 class="text-xs text-gray-500 uppercase tracking-wide">Fournisseurs archivés</h4>
            <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ $fournisseurs->total() }}</p>
        </div>
    </div>
