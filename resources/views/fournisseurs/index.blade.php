...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="mt-1 text-gray-500 text-sm">Gérer la liste de vos fournisseurs.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('fournisseurs.archives') }}"
            ...
            <a href="{{ route('fournisseurs.create') }}"
                ...
                <i class="fa-solid fa-user-plus"></i> Nouveau fournisseur
            </a>
            </div>
    </div>
    ...
    <form action="{{ route('fournisseurs.index') }}" method="GET" id="formRechercheFournisseurs" class="mb-6">
        <div class="relative">
    ...
    <script>
        let timerFournisseur;
        document.getElementById('searchFournisseur').addEventListener('input', function () {
            clearTimeout(timerFournisseur);
            let recherche = this.value;
            timerFournisseur = setTimeout(function () {
                fetch("{{ route('fournisseurs.index') }}?q=" + encodeURIComponent(recherche))
                    .then(response => response.text())
                    .then(html => {
                        let parser = new DOMParser();
                        let doc = parser.parseFromString(html, 'text/html');
                        document.getElementById('tableFournisseurs').innerHTML =
                            doc.getElementById('tableFournisseurs').innerHTML;
                        document.getElementById('paginationFournisseurs').innerHTML =
                            doc.getElementById('paginationFournisseurs')?.innerHTML ?? '';
                    });
            }, 300);
        });
    </script>
   ...
   <div class="bg-white rounded-2xl border border-gray-200/60 shadow-[0_4px_25px_rgba(0,0,0,0.04)] overflow-hidden">
    ...
    <div class="overflow-x-auto max-h-[650px] overflow-y-auto" id="tableFournisseurs">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($fournisseurs as $fournisseur)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4 text-sm font-semibold text-gray-900">{{ $fournisseur->nom }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $fournisseur->email ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $fournisseur->telephone ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $fournisseur->matricule_fiscal ?? '—' }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <form action="{{ route('fournisseurs.archiver', $fournisseur->id) }}" method="POST"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-400 text-sm">Aucun fournisseur trouvé.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
